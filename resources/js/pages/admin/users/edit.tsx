...
import { Department, Position, Role, User } from '@/types';
...
export default function Edit({ user, departments, positions, roles }: EditProps) {
  const form = useForm({
    fname: user.fname || '',
    lname: user.lname || '',
    email: user.email || '',
  ...
  return (
    <AppLayout breadcrumbs={[{ title: 'จัดการผู้ใช้งาน', href: '/admin/users' }]}>
      <Head title={`แก้ไขผู้ใช้: ${user.fname} ${user.lname}`} />
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  <label htmlFor="password" className="text-sm font-medium">
                    รหัสผ่าน <span className="text-muted-foreground text-xs">(เว้นว่างถ้าไม่ต้องการเปลี่ยน)</span>
                  </label>
                  <Input
                    id="password"
                    type="password"
